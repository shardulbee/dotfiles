		...
		sub_mappings = {
			["a"] = {
				...
				fn = function()
					...
					local ok, url, title = hs.osascript.applescript([[
					...
					if not ok or url == "" then
						hs.alert.show("❌ No Chrome window/tab found")
						return
					end
					...
					local pageUrl = parts[1] or ""
					local pageTitle = parts[2] or ""
					...
					-- Send POST request to TurboPins
					hs.http.asyncPost("https://turbopins.tail8779.ts.net/bookmarks", hs.json.encode(bookmarkData), {
						["Content-Type"] = "application/json",
						["Accept"] = "application/json",
					}, function(status, body, headers)
						if status == 201 or status == 200 then
							hs.alert.show("✅ Bookmarked: " .. pageTitle:sub(1, 50))
						else
							hs.alert.show("❌ Failed to bookmark (status: " .. tostring(status) .. ")")
						end
					end)
				end,
			},
