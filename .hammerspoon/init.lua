...
local common = {
    ...
    sub_mappings = {
      ...
      ["c"] = {
        ...
        sub_mappings = {
          ["v"] = {
            alias = "video",
            fn = openUrl("raycast://extensions/Aayush9029/cleanshotx/record-screen"),
          },
          ["c"] = {
            alias = "capture",
            fn = openUrl(
  ...
  ["o"] = {
    ...
    sub_mappings = {
      ...
      ["a"] = { alias = "anki", fn = launchOrFocusApp("Anki") },
    },
  },
}
...
local personal = {
  ["o"] = {
    alias = "open",
    sub_mappings = {
      ["m"] = { alias = "mail", fn = chrome.LaunchOrFocusTab("https://app.fastmail.com/mail/Inbox/") },
      ["c"] = { alias = "calendar", fn = launchOrFocusApp("Fantastical") },
      ["s"] = { alias = "messages", fn = launchOrFocusApp("Messages") },
    },
  },
}
...
local finalMappings
if string.find(hs.host.localizedName(), "turbochardo") then
...
local Watcher = hs.pathwatcher.new(os.getenv("HOME") .. "/.hammerspoon", function(files)
  hs.alert.show("Config change detected")
  hs.reload()
end):start()
hs.alert.show(" ✔︎")
