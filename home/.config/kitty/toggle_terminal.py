...
def is_window_vim(window):
    ...
    return any(
        re.search(VIM_PROCESS_NAME, p["cmdline"]
                  [0] if len(p["cmdline"]) else "", re.I)
        for p in fp
    )
...
def handle_result(
    ...
    if is_window_vim(window):
        if len(tab.windows) == 1:
            # make sure that we are in tall layout before launching new window
            # otherwise the new window might be launched in stack layout
            boss.call_remote_control(
                window, ("goto-layout", "--match=state:focused", "tall")
            )
            boss.call_remote_control(
                None, ("launch", "--type=window",
                       "--cwd=current", "--title=current")
            )
            ...
            boss.call_remote_control(
                window, ("focus-window", "--match=neighbor:right")
            )
    ...
    if not nvim_window:
        ...
        # there is one but but it is not focused and we are in stack.
        # unstack and switch to nvim
        boss.call_remote_control(
            window, ("focus-window", f"--match=id:{nvim_window.id}")
        )
        boss.call_remote_control(
            window, ("goto-layout", "--match=state:focused", "tall")
        )
    elif tab.current_layout.name == "tall":
        # there is one but it is not focused and we are in the tall layout,
        # let's focus it and switch to stack
        boss.call_remote_control(
            window, ("focus-window", f"--match=id:{nvim_window.id}")
        )
        boss.call_remote_control(
