...
- Orb → Sharchy: `nix run .#orb-deploy-sharchy` ([script](scripts/orb-deploy-sharchy)).
- Orb → TurboGadget: `nix run .#orb-deploy-turbogadget` ([script](scripts/orb-deploy-turbogadget), [bootstrap](config/turbogadget-deploy.nix)).
...
Check: `nix flake check path:.`. Deployment tests: `python3 -B scripts/test-deploy-turbogadget.py`.
